...
class SoftAttnDepth(nn.Module):
    ...
    def get_depth_sid(self, depth_labels):
        ...
        return t
    def get_depth_anti_sid(self, depth_labels):
        alpha_ = torch.FloatTensor([self.alpha])
        beta_ = torch.FloatTensor([self.beta])
        t = []
        for K in range(depth_labels):
            K_ = torch.FloatTensor([K])
            t.append(torch.log(torch.exp(alpha_) + (torch.exp(beta_ )- torch.exp(alpha_)) * K_ /(depth_labels)))
        t = torch.FloatTensor(t)
        return t
    def forward(self, input_t, eps=1e-6):
        ...
        elif self.discretization == 'ANTI_SID':
            #print(self.discretization)
            grid = self.get_depth_anti_sid(depth).unsqueeze(0).unsqueeze(2).unsqueeze(3)
        else:
            grid = torch.linspace(
...
class MSDepthDecoder(nn.Module):
    def __init__(self,
                 ...
                 use_skips=True,
                 alpha=1e-3,#change 1e-3 to 1e-2 0.0023  1.7e-4
                 beta=1.0,
                 volume_output=False,
		         discretization='UD'):
        ...
        #before program need unmunt
        # self.conv1 = ConvBlock(128, 1)
        # self.conv2 = ConvBlock(64, 1)
        # self.conv3 = ConvBlock(64, 1)
        # self.conv4 = ConvBlock(32, 1)
    ...
    def forward(self, features):
        ...
        self.outputs[("disp", 3)] = self.depth_layer(x)#self.depth_layer(x)#self.sigmoid (self.conv1(x))

        x, z = self.deconv1(x, features["layer1"])
        # print("deconv1",x.shape)
        self.outputs[("disp", 2)] =self.depth_layer(z) #self.depth_layer(z)#self.sigmoid (self.conv2(x))

        x, z = self.deconv2(x, features["conv3"])
        # print("deconv2",x.shape)
        self.outputs[("disp", 1)] =self.depth_layer(z) #self.depth_layer(z)#self.sigmoid (self.conv3(x))
        ...
        self.outputs[("disp", 0)] =self.depth_layer(z)# self.depth_layer(z)#self.sigmoid (self.conv4(x))
        return self.outputs
