...
class GAT(nn.Module):
    def __init__(self,
        ...
        ########################################################33
        # self.gat_layers.append(GATv2Conv(
        #     512, 256, heads[0],residual=False,activation=self.activation))#32
        # # hidden layers
        # for l in range(1, n_layers):
        #     # due to multi-head, the in_dim = num_hidden * num_heads
        #     self.gat_layers.append(GATv2Conv(
        #     256 * heads[l-1], 128, heads[l], residual=True, activation=self.activation))
        # # output projection
        # self.gat_layers.append(GATv2Conv(
        #     128 * heads[-2], 128, heads[-1], residual=True, activation=self.activation))
        ...
        # # self.fc = nn.Sequential(
        # #     nn.Linear(3, in_feats), nn.Sigmoid())
        self.red = nn.Conv2d(2048, 128, kernel_size=1, stride=1, padding=0, bias=False)#101-50
    ...
    def forward(self, x):#
        ...
        features[-1]=self.red(features[-1])
        return features
