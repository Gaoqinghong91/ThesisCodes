...
import os
import functools
def random_adjacency(width,heigh):
    adj_matrix = np.random.randint(0,2,[width,heigh])
    return adj_matrix
...
###########################################################
adj_next = connected_adjacency(40,32, '4').toarray()#256
adj_rand = random_adjacency(40,32)
src_next, dst_next = np.nonzero(adj_next)
src_rand, dst_rand = np.nonzero(adj_rand)

def gen_randomG(feat):
    g_batch=[]
    batch=feat.shape[0]
    for i in range(batch):
        node_feat=feat[i,:,:].squeeze()
        g = dgl.rand_graph(1280, 1000).to("cuda:0")
...
class GAT_rand(nn.Module):
    ...
    def forward(self, x):#
        ...
        feature_list_next=[]
        feature_list_rand=[]
        b_g_next=genG(src_next, dst_next,feat)
        b_g_rand=gen_randomG(feat)
        # print(b_g)
        for g_next in b_g_next:
            #addlinear_input_layers_s#############
            feature_list_next.append(g_next.ndata["feat"].float())
        for g_rand in b_g_rand:
            #addlinear_input_layers_s#############
            feature_list_rand.append(g_rand.ndata["feat"].float())
           
        for i in range(self.num_layers):
        ##################4###################### 
            for j_next, g_next in enumerate(b_g_next):
                feature_list_next[j_next]=self.gat_layers[i](g_next, feature_list_next[j_next]).flatten(1)
        ##################8######################
            for j_rand, g_rand in enumerate(b_g_rand):
                feature_list_rand[j_rand]=self.gat_layers[i](g_rand, feature_list_rand[j_rand]).flatten(1)        
        for m_next, g_next in enumerate(b_g_next):
            feature_list_next[m_next]=self.gat_layers[-1](g_next, feature_list_next[m_next]).mean(1)
        for m_rand, g_rand in enumerate(b_g_rand):
            feature_list_rand[m_rand]=self.gat_layers[-1](g_rand, feature_list_rand[m_rand]).mean(1)          
        feature_list_next=torch.stack(feature_list_next, dim=0)
        feature_list_rand=torch.stack(feature_list_rand, dim=0)

        feature_list=torch.cat((feature_list_next,feature_list_rand),2)
